...
function BookAnAppointment() {
  ...
  useEffect(() => {
    if (
      ...
      (async () => {
        try {
          const doctorsAddresses = await contractC.getNotAppointedDoctors();
  ...
  function handleBook(index) {
    (async () => {
      try {
        const tx = await contractC.connect(signerC).addAppointment(data[index].address);
        await tx.wait();
  ...
  return (
    <>
      ...
      {data.map((value) => (
        <div key={value.index} className={"flex flex-row justify-between mb-6"}>
          <span>{value.name}</span>
          <span>{value.email}</span>
          <span>{value.speciality}</span>
          <span>{value.phone}</span>
          <span>{value.gender}</span>
          <button onClick={() => handleBook(value.index)}>book</button>
        </div>
      ))}
    </>
  );
